...
st.markdown("""
    ...
    <div class="small-title">{ 파미-1 물류 주문서 }<br><br></div>
...
def process_file(sheet_url):
    ...
    preview_df.columns = ["주문번호", "약국상호", "주문수량"]
...
with col2:
    if st.button("📥 주문서 생성"):
        ...
        st.success(f"주문서 생성 완료!! {row_count}개의 주문이 처리되었습니다.")
        st.info(f"적용시간 : {now}")
        ...
        st.warning("👉 위 버튼을 누르시면 최신 데이터를 적용한 물류 주문서를 생성합니다!!")
